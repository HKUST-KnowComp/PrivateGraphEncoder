...
parser.add_argument('--model', type=str, default='GAEMI',
                    choices=['APGE', 'GAEMI'])
...
def main():
    ...
    model = GAEMI(in_dim=features.shape[1], hidden1_dim=args.hidden1, hidden2_dim=args.hidden2,
                  attr_dim=n_class, device=device).to(device)
    ...
    for epoch in range(args.epoch):
        ...
        if (epoch+1) % 10 == 0:
            print("loss attacker: {}".format(loss_att.detach()))
            print_info()
